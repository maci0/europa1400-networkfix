...
- **A/B testing:** `NETWORKFIX_DISABLE=1 docker compose up` = baseline (same ASI, network hooks off). `HARNESS_EVT_GUARD=1` (default on in compose) = env-gated NULL guard on evt poll `0x429800` in networkfix.asi.
- **Fresh prefix inside the image:** `Dockerfile` `wineboot --init` + silent installer → baked `/home/gilde/pfx` (`WINEARCH=win32`, `game.ini cur_res=2 → 1024x768`).
