...
`CONFIG_NET_SCH_NETEM=m` is built as `sch_netem.ko.zst`, but running kernel is `7.1.4-1-cachyos` while installed `linux-cachyos` is `7.1.6-1` — `/usr/lib/modules/7.1.4-1-cachyos` missing (only `7.1.6-1-cachyos` + `6.18-lts` present). Hence `tc qdisc add ... netem` → `qdisc kind is unknown` on host and via `sudo nsenter` for `gilde-host`/`gilde-client` (`CAP_NET_ADMIN` present).
